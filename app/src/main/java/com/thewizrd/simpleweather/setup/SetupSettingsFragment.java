        ...
        onGoingPref.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                ...
                if (value && getSettingsManager().useFollowGPS() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && !getSettingsManager().requestedBGAccess() &&
                        !LocationPermissionRequestHelperKt.backgroundLocationPermissionEnabled(requireActivity())) {
                    runWithView(() -> {
                        Snackbar snackbar = Snackbar.make(
                                LocationPermissionRequestHelperKt.getBackgroundLocationRationale(requireActivity()),
                                Snackbar.Duration.VERY_LONG
                        );
                        snackbar.setAction(android.R.string.ok, view -> {
                            if (shouldShowRequestPermissionRationale(Manifest.permission.ACCESS_BACKGROUND_LOCATION)) {
                                LocationPermissionRequestHelperKt.openAppSettingsActivity(requireActivity());
                            } else {
                                LocationPermissionRequestHelperKt.requestBackgroundLocationPermission(SetupSettingsFragment.this, 0);
                            }
                        });
