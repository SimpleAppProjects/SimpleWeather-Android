...
class WeatherApiRadarViewProvider(context: Context, rootView: ViewGroup) :
    ...
    private val radarLayers: MutableMap<ZonedDateTime, TileOverlay>
    ...
    private fun addLayer(mapFrame: RadarFrame) {
        ...
        if (!radarLayers.containsKey(mapFrame.timestamp)) {
            val overlay = googleMap!!.addTileOverlay(
                TileOverlayOptions().tileProvider(WeatherApiTileProvider(context, mapFrame))
                    .transparency(1f)
            )
            if (overlay != null) {
    ...
    private fun changeRadarPosition(pos: Int, preloadOnly: Boolean = false) {
        ...
        if (nextOverlay != null) {
            nextOverlay.transparency = DEFAULT_TRANSPARENCY
        }
    ...
    private class WeatherApiTileProvider(context: Context, private val mapFrame: RadarFrame?) :
        ...
        override fun getTileUrl(x: Int, y: Int, zoom: Int): String? {
            ...
            if (mapFrame != null) {
                ...
                val url =
                    "https://weathermaps.weatherapi.com/precip/tiles/%s%02d/%d/%d/%d.png".format(
                        date,
                        hour,
                        zoom,
                        x,
