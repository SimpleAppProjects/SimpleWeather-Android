...
/**
 * A page indicator for {@link ViewPager2} based on {@link
 * androidx.wear.widget.drawer.PageIndicatorView} and {@link
 * com.google.android.material.tabs.TabLayoutMediator}
 *
 * <p>Use {@link #setPager(ViewPager2)} to connect this view to a pager instance.
 */
public class PageIndicatorView2 extends View {
    ...
    public PageIndicatorView2(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mCurrentViewPagerState = SCROLL_STATE_IDLE;
        if (isInEditMode()) {
    ...
    /**
     * Supplies the ViewPager2 instance, and attaches this view to the pager.
     *
    ...
    public void setPager(ViewPager2 pager) {
        ...
        if (mAdapter == null) {
            throw new IllegalStateException(
                    "PageIndicatorView attached before ViewPager2 has an " + "adapter");
        }

        // Add our custom OnPageChangeCallback to the ViewPager
        pager.registerOnPageChangeCallback(new PageIndicatorOnPageChangeCallback(this));
        mAdapter.registerAdapterDataObserver(new PagerAdapterObserver());

        updateNumberOfPositions();

        // Now update the scroll position to match the ViewPager's current item
        positionChanged(pager.getCurrentItem());
    }
    ...
    private static class PageIndicatorOnPageChangeCallback extends ViewPager2.OnPageChangeCallback {
        @NonNull
        private final WeakReference<PageIndicatorView2> pageIndicatorRef;

        PageIndicatorOnPageChangeCallback(PageIndicatorView2 pageIndicator) {
            pageIndicatorRef = new WeakReference<>(pageIndicator);
        }

        @Override
        public void onPageScrollStateChanged(final int state) {
            PageIndicatorView2 pageIndicator = pageIndicatorRef.get();
            if (pageIndicator != null) {
                if (pageIndicator.mCurrentViewPagerState != state) {
                    pageIndicator.mCurrentViewPagerState = state;
                    if (pageIndicator.mDotFadeWhenIdle) {
                        if (state == SCROLL_STATE_IDLE) {
                            if (pageIndicator.mVisible) {
                                pageIndicator.fadeOut(pageIndicator.mDotFadeOutDelay);
                            } else {
                                pageIndicator.fadeInOut();
                            }
                        }
                    }
                }
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            ...
            if (pageIndicator != null) {
                if (pageIndicator.mDotFadeWhenIdle) {
                    if (pageIndicator.mCurrentViewPagerState == SCROLL_STATE_DRAGGING) {
                        if (positionOffset != 0) {
                            if (!pageIndicator.mVisible) {
        ...
        public void onPageSelected(final int position) {
            PageIndicatorView2 pageIndicator = pageIndicatorRef.get();
            if (pageIndicator != null) {
                if (position != pageIndicator.mSelectedPosition) {
                    pageIndicator.positionChanged(position);
                }
            }
        }
    }
